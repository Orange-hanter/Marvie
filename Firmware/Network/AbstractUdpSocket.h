...
public:
	...
	virtual uint32_t writeDatagram( const uint8_t* data, uint32_t size, IpAddress address, uint16_t port ) = 0;
	virtual uint32_t writeDatagram( const uint8_t* data, uint32_t size ) = 0;
