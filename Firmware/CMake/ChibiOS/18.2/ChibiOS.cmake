...
IF(NOT CHIBIOS_CONFIG_DIR)
    set(CHIBIOS_CONFIG_DIR ".")
ENDIF()

IF(NOT CHIBIOS_HALCONF_FILE)
    MESSAGE(STATUS "No ChibiOS halconf.h specified, trying to find it...")
    FILE(GLOB CHIBIOS_HALCONF_FILE "${CHIBIOS_CONFIG_DIR}/halconf.h")
...
IF(NOT CHIBIOS_HALCONF_COMMUNITY_FILE)
    MESSAGE(STATUS "No ChibiOS halconf_community.h specified, trying to find it...")
    FILE(GLOB CHIBIOS_HALCONF_COMMUNITY_FILE "${CHIBIOS_CONFIG_DIR}/halconf_community.h")
    IF(CHIBIOS_HALCONF_COMMUNITY_FILE STREQUAL "")
        MESSAGE(STATUS "Cannot find halconf_community.h")
    ELSE()
        MESSAGE(STATUS "Found halconf_community.h: ${CHIBIOS_HALCONF_COMMUNITY_FILE}")
    ENDIF()
ENDIF()

IF(NOT CHIBIOS_CHCONF_FILE)
    MESSAGE(STATUS "No ChibiOS chconf.h specified, trying to find it...")
    FILE(GLOB CHIBIOS_CHCONF_FILE "${CHIBIOS_CONFIG_DIR}/chconf.h")
...
IF(CHIBIOS_HALCONF_COMMUNITY_FILE)
    FILE(STRINGS ${CHIBIOS_HALCONF_COMMUNITY_FILE} HALCONF_COMMUNITY_LINES REGEX "#define HAL_USE_([a-zA-Z_0-9]+) +TRUE")
    FOREACH(LINE ${HALCONF_COMMUNITY_LINES})
        STRING(REGEX REPLACE "#define HAL_USE_([a-zA-Z_0-9]+) +TRUE" "\\1" COMP ${LINE})
        LIST(APPEND CHIBIOS_COMMUNITY_HAL_COMPONENTS ${COMP})
    ENDFOREACH()
ENDIF()
...
INCLUDE(ChibiOS/18.2/ChibiOS_Community)
