...
class Tem104USensor : public AbstractBRSensor
{
public:
	...
	public:
		...
		struct IntegratedValues
		{
			...
			uint32_t T_Wrk;     // общее время работы
			uint32_t T_Cnt;		// время наработки без ошибок
	...
	explicit Tem104USensor();
	virtual ~Tem104USensor();
