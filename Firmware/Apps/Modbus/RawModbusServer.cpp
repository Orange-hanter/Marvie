...
RawModbusServer::Stream::Stream()
{
	io = nullptr;
}

int RawModbusServer::Stream::read( uint8_t* buffer, size_t buffer_size )
{
...
int RawModbusServer::Stream::write( uint8_t* buffer, size_t len )
{
	return io->write( buffer, len, TIME_IMMEDIATE );
}

void RawModbusServer::Stream::txEnable( bool state )
{

}

bool RawModbusServer::Stream::writeComplete()
{
	return io->waitForBytesWritten( TIME_IMMEDIATE );
}

void RawModbusServer::Stream::communicationStatus( bool rx, bool tx )
{
