...
{ \
	...
	for( uint16_t i = 0; i < 4; ++i )
	{
		...
		io->write( readTimerRequest, sizeof( readTimerRequest ) );
		returnIfError( waitResponse(), 2 + i );
		parseRamResponse( i );
	}
...
SensorData::Error Tem104Sensor::waitResponse()
{
	if( !waitForReadAvailable( 6, TIME_MS2I( 1000 ) ) )
		return SensorData::Error::NoResponseError;
	uint8_t addr[2];
	io->peek( 1, addr, 2 );
	if( addr[0] != address || addr[1] != uint8_t( ~address ) )
	{
	...
	uint8_t len;
	io->peek( 5, &len, 1 );
	if( !waitForReadAvailable( len + 6 + 1, TIME_MS2I( 1000 ) ) )
		return SensorData::Error::NoResponseError;
...
void Tem104Sensor::parseTimerResponse()
{
	...
	io->read( &data.integrated.date[0], 8, TIME_IMMEDIATE );
	{
		...
		for( auto i = 0; i < 4; ++i )
		{
			data.integrated.ch[i].intV = ( double )( low.V[i] );
			data.integrated.ch[i].intM = ( double )( low.M[i] );
			data.integrated.ch[i].intQ = ( double )( low.Q[i] );
		}
	}
	{
		struct
		...
		for( auto i = 0; i < 4; ++i )
		{
			data.integrated.ch[i].intV += ( double )( hi.V[i] );
			data.integrated.ch[i].intM += ( double )( hi.M[i] );
			data.integrated.ch[i].intQ += ( double )( hi.Q[i] );
		}
	}
	io->read( ( uint8_t* )&data.integrated.TRab, 4, TIME_IMMEDIATE );
	{
		...
		for( auto i = 0; i < 4; ++i )
		{
			data.integrated.ch[i].TNar = dataErr.TNar[i];
			data.integrated.ch[i].Tmin = dataErr.Tmin[i];
			data.integrated.ch[i].Tmax = dataErr.Tmax[i];
		}
	}
	{
		struct
		...
		for( auto i = 0; i < 4; ++i )
		{
			data.integrated.ch[i].Tdt = dataErr.Tdt[i];
			data.integrated.ch[i].Ttn = dataErr.Ttn[i];
	...
	}
	{
		...
		for( auto i = 0; i < 4; ++i )
		{
			data.integrated.ch[i].teherr = dataTmp.teherr[i];
			for( auto j = 0; j < 3; ++j )
			{
				data.integrated.ch[i].t[j] = dataTmp.tmp[i][j];
...
void Tem104Sensor::parseRamResponse( uint32_t ch )
{
	...
	io->read( nullptr, 6, TIME_IMMEDIATE );
	{
		...
		for( auto i = 0; i < 4; ++i )
		{
			data.instant.ch[ch].tmp[i] = ram.tmp[i];
			data.instant.ch[ch].prs[i] = ram.prs[i]; // sempai takoy zabotliviy
			data.instant.ch[ch].ro[i] = ram.ro[i];
		}
	}
	{
		struct
		...
		for( auto i = 0; i < 4; ++i )
		{
			data.instant.ch[ch].hent[i] = ram.hent[i];
			data.instant.ch[ch].rshv[i] = ram.rshv[i]; // sempai takoy zabotliviy x2
			data.instant.ch[ch].rshm[i] = ram.rshm[i];
		}
	}
	{
#pragma pack( push, 1 )
...
#pragma pack( pop )
		io->read( ( uint8_t* )&ram, sizeof( ram ), TIME_IMMEDIATE );
		for( auto i = 0; i < 4; ++i )
			data.instant.ch[ch].pwr[i] = ram.pwr[i];
		data.instant.ch[ch].tekerr = ram.tekerr;
		data.instant.ch[ch].teherr = ram.teherr;
	}
